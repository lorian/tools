...
# Remove these strains from Martin file
...
if filename.startswith('Martin'):
	duplicates = i100_species
else:
	duplicates = []
...
for line in mfa:
	if line[0] == '>': # fasta name
		...
		if last_genome: # if we have a genome in buffer, we need to chop it and write it before handling the new entry

			# Calculate most balanced length of genome fragments, to avoid having particularly short fragments
			num_of_fragments = len(last_genome)/fragment_size # auto-rounded to nearest int
			length_of_fragments = int(math.ceil(len(last_genome)/float(max(num_of_fragments,1)))) # rounded up to nearest bp
#			print "Species: {0}\tGenome length: {1}\t# Fragments: {2}\tFragment length: {3}".format(last_species,len(last_genome),num_of_fragments,length_of_fragments)

			for i in range(0,len(last_genome),length_of_fragments):
				fa_genomes.write(last_species[:-1] + str(i) +'\n') #insert char number in fasta name
				fa_genomes.write(last_genome[i:i+length_of_fragments]+'\n')

			last_genome = "" # wipe last genome once it's written
		...
		if line.find('VIRL') != -1: #ignore viruses
			...
			fa_plasmids.write(line)
				...
				# skip duplicates
		if lanthpy.genome_name_cleanup([line]):
			if any(species in line for species in duplicates):
				skip_current = True
			else:
				last_type = 'genome'
				last_species = line # for purposes of labeling the genome fragments
		...
		if last_type == 'genome' && not skip_current: # first store entire genome in variable
			last_genome += line
