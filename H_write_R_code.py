...
conditions = ['hES','teratoma','organoid','differentiated','control','background']
...
#	print "write.csv( ensembl_cds_binom[ order(ensembl_cds_binom$pval), ], file=\"{0}_{1}_filtered_genes.csv\" )".format(pair[0],pair[1])
	...
	print "write.csv( ensembl_cds_binom[ order(ensembl_cds_binom$pval), ], file=\"{0}_{1}_all_genes.csv\" )".format(pair[0],pair[1])
