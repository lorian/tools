...
def process_input(filename,size,fragmented=False):
	...
	if suffix == 'xprs': #express file
		...
		raw_est.counts = numpy.array([float(i) for i in zip(*input_data)[4]])
		raw_est.abundance = numpy.array([float(i) for i in zip(*input_data)[3]])

	elif suffix == 'txt': #gasic file - NOT WORKING
		input_species = zip(*input_data)[0] # gasic species names are in first column
		input_counts = numpy.array([float(i) for i in zip(*input_data)[2]])
...
#							print "Error: species match for {0} not found in data. Setting abundance and counts to 0 for {0}.".format(sp)
	...
	diff = 100*(numpy.array(adjusted_abundance) - numpy.array(true_abundance))/numpy.array(true_abundance)
