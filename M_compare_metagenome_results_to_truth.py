...
def dataset_truth(dataset):
	...
	if dataset == 'i100':
		try:
			...
			i100_csv = [r for r in csv.reader(open('/home/lorian/tools/i100_truth.csv','r'), 'excel')] # when on server
		...
		truth.set_by_array(list(i100_csv))
	elif dataset == 'i100_noa':
		i100_csv = [r for r in csv.reader(open('/home/lanthala/compbio_tools/i100_truth_noa.csv','r'), 'excel')]
		truth.set_by_array(list(i100_csv))
	elif dataset == 'i400':
...
def calc_kraken_error(truth,est,rank):
	...
	for name in rank_names:
		#if (name not in truth.species) and (norm_est.lookup_count(name) > 10):
			#print "{}: {}".format(tax_dict.get_taxa_by_id(name).name,norm_est.lookup_count(name))
	...
	for sp in norm_est.species:
		if sp not in rank_names: # look for things assigned above tier
			...
			if norm_est_sum > true_sum: # only counts as wrong if it overnorm_estimates
				#print "\t\t\t\t{}".format(sp)
				#print tax_dict.get_taxa_by_id(sp)
				#print climb_tree_verbose(sp,truth)
				#print climb_tree_verbose(sp,norm_est)
...
def main(argv=sys.argv):
	...
	Command line usage: python M_compare_metagenomic_results_to_truth.py
						[filename] [dataset (defaults to i100)] <-g show graphs?>
