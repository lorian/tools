...
def main():
	...
	cleaned_fasta_file = insert_suffix(raw_fasta_file, '_poundless')
	...
	sorted_fasta_file = insert_suffix(raw_fasta_file, '_sorted')
	...
	all_fastas = [insert_suffix(sorted_fasta_file,'_genomes_0','mfa'),
	              ...
	              insert_suffix(sorted_fasta_file,'_plasmids_0','mfa')]
	...
	# Merge sam files
	merged_sam_file = "{0}_{1}".format(test_basename,i100_alignment.rpartition("test")[2])
	...
	merged_all_fastas = insert_suffix(sorted_fasta_file,"_i100")
	if missing_file(merged_all_fastas):
		script.write('cat {0} {1} > {2} \\\n&& '
		             .format(i100_fasta," ".join([f for f in all_fastas]),merged_all_fastas))

	# Run express (assuming this will always be run, otherwise why run the script)
	script.write('express -f {0} --max-indel-size 100 -B {1} {2} {3}'
	             .format(express_f,express_cycles,merged_all_fastas,sorted_bam_file))
